...
class VisitedLocationResource extends Resource
{
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('visitable_type')
                    ...
                    ->formatStateUsing(function ($state, $record) {
                        // Безопасная обработка для пустой таблицы
                        if (!$record) {
                            return '❓ Нет данных';
                        }
                        ...
                        if (!class_exists($state)) {
                            return '🗄️ Удалено: ' . class_basename($state);
                        }
                    ...
                    ->colors([
                        'success' => fn ($state) => $state === 'App\\Models\\Shift',
                        'warning' => fn ($state) => $state === 'App\\Models\\MassPersonnelReport',
                ...
                Tables\Columns\TextColumn::make('visitable_id')
                    ...
                    ->formatStateUsing(function ($state, $record) {
                        ...
                        if (empty($record->visitable_type) || !is_string($record->visitable_type)) {
                            return '#' . $state . ' (архив)';
                        }
                        
                        if (!class_exists($record->visitable_type)) {
                            return '#' . $state . ' (' . class_basename($record->visitable_type) . ' удален)';
                        }
                        
                        try {
                            $visitable = $record->visitable;
                            if (!$visitable) {
                                return '#' . $state . ' (не найден)';
                            }
                            
                            return match (get_class($visitable)) {
                                'App\\Models\\Shift' => 'Смена #' . $state . ' - ' . ($visitable->user?->full_name ?? 'Без исполнителя'),
                                'App\\Models\\MassPersonnelReport' => 'Отчет #' . $state,
                                'App\\Models\\Expense' => 'Расход #' . $state . ' - ' . ($visitable->amount ?? 0) . ' ₽',
                                'App\\Models\\ContractorWorker' => 'Работник #' . $state . ' - ' . ($visitable->name ?? 'Без имени'),
                                default => '#' . $state . ' (' . class_basename($visitable) . ')',
                            };
                        } catch (\Exception $e) {
                            return '#' . $state . ' (ошибка связи)';
                        }
                    })
                    ->url(function ($record) {
                        if (!$record) {
                        ...
                        if (empty($record->visitable_type) || !is_string($record->visitable_type) || !class_exists($record->visitable_type)) {
                            return null;
                        }
                        ...
                        try {
                            ...
                            return match (get_class($visitable)) {
                                ...
                                'App\\Models\\Expense' => \App\Filament\Resources\ExpenseResource::getUrl('edit', [$record->visitable_id]),
                                'App\\Models\\ContractorWorker' => \App\Filament\Resources\ContractorWorkerResource::getUrl('edit', [$record->visitable_id]),
                                default => null,
                            };
                        } catch (\Exception $e) {
                            return null;
                ...
                Tables\Columns\TextColumn::make('duration_minutes')
                    ->label('Длительность')
                    ->formatStateUsing(fn ($state) => ($state ?? 0) . ' мин.')
                    ->sortable()
                    ->alignCenter(),
                ...
                Tables\Columns\TextColumn::make('workers_count')
                    ...
                    ->visible(fn ($record): bool => 
                        $record && $record->visitable_type === 'App\\Models\\MassPersonnelReport'
                    ),
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('view_photos')
                    ...
                    ->color('gray')
                    ->url(fn ($record) => $record ? self::getUrl('edit', [$record->id]) . '?activeRelationManager=0' : null),
                ...
                Tables\Actions\Action::make('open_map')
                    ...
                    ->openUrlInNewTab()
                    ->hidden(fn ($record): bool => !$record || !$record->latitude || !$record->longitude),
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        // Проверяем, есть ли записи перед загрузкой связей
        if ($query->count() > 0) {
            $query->with(['visitable' => function ($morphTo) {
                $morphTo->morphWith([
                    \App\Models\Shift::class => ['user', 'workRequest'],
                    \App\Models\MassPersonnelReport::class => ['workRequest'],
                    \App\Models\Expense::class => ['category'],
                    \App\Models\ContractorWorker::class => ['contractor'],
                ]);
            }]);

            // Фильтруем только валидные типы
            $validTypes = [
                'App\\Models\\Shift',
                'App\\Models\\MassPersonnelReport',
                'App\\Models\\Expense',
            ...
            $query->whereIn('visitable_type', $validTypes);
        }
